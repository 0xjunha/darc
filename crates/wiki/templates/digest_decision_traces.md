...
## Curated Playbook
Use the examples below as preferred patterns, not as a mandatory fixed sequence.
...
### 3. Discover Decision-Shaped Evidence Across Sessions
```bash
...
### 4. Follow File Arcs Across Sessions
```bash
...
### 5. Check Existing Wiki Coverage Before Proposing Duplicates
```bash
...
### 6. Verify Finality Against Repository And History When Needed
```bash
