...
Git transport choices were evaluated:
...
- `git2` is a mature binding to libgit2. It directly supports repository read/write, fetch, push, remote callbacks,
  credential helpers, and SSH agent credentials. It is not pure Rust, and in practice it does not always match the
  user's installed Git client behavior for HTTPS helpers, platform keychains, enterprise SSO, proxies, and SSH config.
- The system `git` executable is accepted for V1 transport and local share-cache repository operations. This makes the
  expected auth contract easy to explain: if the equivalent non-interactive `git fetch` or `git push` works for the
  configured remote, Darc share should work too. Darc still owns artifact validation, cache-root safety, redaction, and
  encryption; Git owns transport, credentials, and remote protocol behavior.
...
- Darc uses the local `git` executable for share-cache `init`, `fetch`, `checkout`, `reset`, `clean`, `commit`, and
  `push`. Darc runs Git non-interactively and expects credentials to be available through the user's normal Git setup,
  such as SSH agent keys or HTTPS credential helpers. Darc does not prompt for or store GitHub tokens.
- When the active project falls back to `origin`, Darc stores the configured remote URL in the share cache but resolves
  Git URL rewrites for project keys, cache keys, and shared provenance. This preserves `insteadOf` ergonomics without
  persisting credential-expanded URLs generated by Git config.
...
- Git CLI docs: `fetch`, `push`, `checkout`, `reset`, `clean`, `commit`, credential helpers, SSH config, and refspecs.
